            ...
            for audio_tensor in audio_tensors:
                if audio_tensor.shape[0] < max_length:
                    ...
                    padded_audio = torch.cat([
                        audio_tensor, 
                        torch.zeros(max_length - audio_tensor.shape[0])
                    ])
                else:
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        await batcher.start()
        logger.info(f"BEATs classifier and MicroBatcher initialized (max_batch_size={max_batch_size}, max_wait_ms={max_wait_ms})")
    except Exception as e:
    ...
    try:
        ...
        structured_response = {
            ...
            "summary": {
                ...
                "successfully_processed": len(classification_results)
            }
        }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model-path",default="/inspire/hdd/project/embodied-multimodality/public/hfchen/cargoflow/cargoflow/src/cargoflow/llms/beats/BEATs_iter3_plus_AS2M_finetuned_on_AS2M_cpt2.pt", help="Path to BEATs model checkpoint")
